...
<body class="bg-gray-50 font-sans">
            ...
            <div class="flex items-center gap-3 bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                ...
                <div>
                    <h1 class="font-bold text-2xl text-gray-800 flex items-center gap-2">
                        <i class="fas fa-calendar-days text-orange-500"></i> Detail Event
                    </h1>
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-800">{{ $event->no_telpon }}</p>
                    </div>
                </div>
                <div>
                    <p class="text-xs text-gray-400 uppercase font-bold mb-2">Foto KTP</p>
                    @if ($event->ktp)
                        <img src="{{ asset('storage/' . $event->ktp) }}" alt="Foto KTP"
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <p class="font-semibold text-gray-800">{{ $event->kategori->name ?? '-' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-800">{{ $event->start_time }} &rarr; {{ $event->end_time }}</p>
                    </div>
                </div>
                <div>
                    <p class="text-xs text-gray-400 uppercase font-bold mb-1">Location</p>
                    <p class="font-semibold text-gray-800 mb-2">{{ $event->location }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        @if ($event->img)
                            <img src="{{ asset('storage/' . $event->img) }}" alt="Poster 1"
                    ...
                    <div>
                        ...
                        @if ($event->img2)
                            <img src="{{ asset('storage/' . $event->img2) }}" alt="Poster 2"
                                class="w-full rounded-lg border border-gray-200">
                        @else
                            <p class="text-gray-500">Tidak ada gambar</p>
                        @endif
    ...
    <script>
        ...
        var locationString = @json($event->location);
        if (locationString) {
